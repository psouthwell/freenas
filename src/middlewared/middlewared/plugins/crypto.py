...
class CertificateService(CRUDService):
    ...
    async def do_create(self, data):
        ...
        await validate_cert_name(
            ...
            verrors, f'certificate_create.name'
        )
    ...
    async def do_update(self, id, data):
        ...
        required_fields = ['certificate', 'privatekey']
        ...
        if old['type'] == CERT_TYPE_CSR:
            if new['privatekey']:
                # THE TYPE SHOULD ONLY BE CHANGED WHEN A PRIVATE KEY HAS BEEN ADDED TO THE CSR ?
...
class CertificateAuthorityService(CRUDService):
    ...
    def _set_enum(name):
        def set_enum(attr):
            attr.enum = ['CA_CREATE_INTERNAL', 'CA_CREATE_IMPORTED', 'CA_CREATE_INTERMEDIATE', 'CA_SIGN_CSR']
    ...
    # CA_SIGN_CSR                 - ca_sign_csr  # TODO: THIS SHOULD NOT BE CALLED WITH DO_CREATE, REMOVE FROM ENUM
    ...
    async def do_create(self, data):
        ...
        await validate_cert_name(
            self.middleware, data['name'], self._config.datastore,
            verrors, f'certificate_authority_create.name'
        )
        ...
        data = await self.map_functions[create_type](data)
    ...
    @accepts(
        Patch(
            'ca_create', 'ca_sign_csr',
            ('edit', _set_required('csr_cert_id')),
            ('edit', _set_required('ca_id')),
        ),
        Str('schema_name'),
    )
    async def ca_sign_csr(self, data, schema_name):
        ...
        # IS THIS NECESSARY ?
        await self.validate_common_attributes(ca_data, schema_name)
        ...
        new_PKey = crypto.dump_privatekey(crypto.FILETYPE_PEM, PKey).decode()
        new_csr = {
    ...
    @accepts(
        Patch(
            'certificate_create_internal', 'ca_create_intermediate',
            ('edit', _set_required('signedby')),
        ),
    )
    ...
    @accepts(
        Patch(
            'certificate_create', 'ca_create_imported',
            ('edit', _set_required('certificate')),
    ...
    @accepts(
        Patch(
            'certificate_create_internal', 'ca_create_interal',
            ('rm', {'name': 'signedby'})
        )
    )
    async def __create_internal(self, data):
    ...
    async def do_update(self, id, data):
        ...
        if data.get('create_type') == 'CA_SIGN_CSR':
            data['ca_id'] = id
        ...
        required_fields = ['certificate', 'name']  # SHOULD MORE FIELDS BE CHECKED TO ENSURE THAT THEY EXIST?
        for field in required_fields:
            if not new.get(field):
                verrors.add(
        ...
        start_ssl = data.get('start_ssl') if 'start_ssl' in data else True
        if start_ssl:
            await self.middleware.call(
                'service.start',
                'ix-ssl',
